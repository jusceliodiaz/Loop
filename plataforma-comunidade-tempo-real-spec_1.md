# Plataforma de Comunidade em Tempo Real — Especificação Técnica

> **Codinome interno:** `Factory`
> **Versão do documento:** 1.0
> **Data:** 22/08/2026
> **Status:** Draft para revisão de arquitetura
> **Escopo:** Especificação funcional, técnica e de execução para uma plataforma de comunicação em tempo real com camada de aplicações colaborativas integradas.
...
| 22  | [Plataforma de aplicações em canal](#22-plataforma-de-aplicações-em-canal) |
...
Times criativos e técnicos hoje operam fragmentados entre quatro ferramentas: Discord (comunicação informal e voz), Slack (comunicação de trabalho), Notion (documentação) e Teams/Meet (reuniões). O custo dessa fragmentação não é a assinatura — é o **contexto perdido**: a decisão foi tomada na call, registrada em nenhum lugar, referenciada num arquivo que está no Drive, e a task que saiu dela vive num board que ninguém abre.

### 1.2 Proposta

Uma plataforma onde o **canal é a unidade de contexto**, não a conversa. Cada canal carrega abas de aplicação (`Chat`, `Files`, `Tasks`, `3D Viewer`, `Meetings`, `AI Assistant`) que compartilham o mesmo espaço de permissões, os mesmos membros e o mesmo histórico. A conversa deixa de ser um fluxo isolado e passa a ser uma das visões de um espaço de trabalho.
...
| ------- | -------------------------------------------- | -------------------------------------------------------------- |
| Discord | Voz sempre-ativa, comunidade, latência baixa | Zero estrutura de trabalho; sem tasks/docs; permissões rígidas |
| Slack   | Integrações, threads maduras                 | Voz fraca, caro por assento, histórico limitado no free        |
| Notion  | Documentos e bases estruturadas              | Tempo real fraco, sem voz                                      |
| Teams   | Enterprise, compliance                       | UX pesada, hostil a comunidades abertas                        |

**Nossa aposta:** a ergonomia de voz do Discord + a estrutura de trabalho do Notion, num único modelo de permissões.

### 1.4 Personas

| Persona                           | Perfil                           | Necessidade central                    | Métrica de sucesso                      |
| --------------------------------- | -------------------------------- | -------------------------------------- | --------------------------------------- |
| **Juscelio** — Technical Artist   | Lidera estúdio de 12 pessoas     | Revisar assets 3D sem sair da conversa | Reviews concluídas dentro da plataforma |
| **Ana** — Product Owner           | Coordena 3 squads                | Rastrear decisões até a task           | Decisões com task vinculada             |
| **Carlos** — Membro de comunidade | Comunidade pública de 4k membros | Achar informação sem se perder         | Retenção D30                            |
| **Marcos** — Moderador voluntário | Modera comunidade grande         | Agir rápido com trilha auditável       | Tempo médio até ação de moderação       |

### 1.5 Princípios de produto

1. **Tempo real por padrão.** Nenhuma tela exige refresh. Se um dado mudou no servidor, ele muda na tela em menos de um segundo.
2. **Permissão é uma coisa só.** Um único motor de permissões governa chat, arquivos, voz e apps. Nada de ACL paralela por feature.
3. **Otimismo na UI, verdade no servidor.** A ação aparece instantânea; o servidor reconcilia e, se recusar, a UI reverte de forma visível.
4. **Histórico é ativo, não passivo.** Buscar, fixar, referenciar e citar são operações de primeira classe.
5. **Extensível sem fork.** Apps de canal usam SDK público e sandbox — nós mesmos construímos os primeiros apps usando esse SDK.
...
- Mensageria em tempo real com edição, exclusão, respostas, threads, reactions, menções, fixação e busca.
- Mensagens diretas 1:1 e grupos privados.
- Sistema de amizades e bloqueio.
- Cargos, permissões hierárquicas em três níveis (servidor → categoria → canal) e overrides por usuário.
...
| ------------------------------------------ | ---------------------------------------------- | ------------------------------- |
| Aplicativo desktop nativo (Electron/Tauri) | PWA cobre 90% do caso de uso inicial           | Após 5k MAU                     |
| Apps mobile nativos                        | Custo de manutenção de 3 clientes              | Fase 4                          |
| Federação / ActivityPub                    | Complexidade de moderação distribuída          | Não planejado                   |
| E2E encryption em canais de servidor       | Incompatível com busca server-side e moderação | Apenas para DMs, fase 4         |
| Marketplace de bots de terceiros           | Sem massa crítica                              | Após plataforma de apps estável |
| Monetização (boosts, nitro)                | Foco em product-market fit                     | Fase 4                          |
| Compliance SOC2 / HIPAA                    | Custo desproporcional pré-receita              | Sob demanda enterprise          |
| Streaming Go-Live para audiência grande    | SFU→CDN é projeto próprio                      | Fase 4                          |

### 2.3 Restrições assumidas

- Time inicial: 2 a 4 desenvolvedores.
- Orçamento de infra inicial: até US$ 300/mês.
- Meta de latência de mensagem (p95, mesma região): **≤ 250 ms** do envio ao recebimento por terceiros.
- Meta de latência de voz (mouth-to-ear, p95): **≤ 200 ms**.
- Alvo inicial de escala: **500 usuários simultâneos**, arquitetura preparada para 50k sem reescrita estrutural.
...
| Termo                | Definição                                                                              |
| -------------------- | -------------------------------------------------------------------------------------- |
| **Servidor** (guild) | Comunidade isolada; unidade de tenancy lógica. Contém canais, cargos e membros.        |
| **Categoria**        | Agrupador de canais; carrega overrides de permissão herdados pelos filhos.             |
| **Canal**            | Espaço de conversa. Tipos: `text`, `voice`, `announcement`, `forum`, `stage`.          |
| **Membro**           | Relação entre usuário e servidor. Carrega apelido, cargos e data de entrada.           |
| **Cargo** (role)     | Conjunto nomeado de permissões, com cor e posição hierárquica.                         |
| **Override**         | Permissão de allow/deny aplicada a um cargo ou usuário num canal/categoria específico. |
| **Thread**           | Canal efêmero filho de uma mensagem, com lista de participantes própria.               |
| **Presença**         | Estado volátil de conexão do usuário: `online`, `idle`, `dnd`, `offline`.              |
| **Sessão**           | Uma conexão WebSocket ativa. Um usuário pode ter N sessões.                            |
| **App de canal**     | Extensão que renderiza uma aba dentro de um canal, em sandbox.                         |
...
1. Todo servidor tem exatamente um `owner_id` válido e ao menos um canal de texto.
2. Todo servidor tem um cargo `@everyone` com `position = 0`, não deletável e não removível de nenhum membro.
3. Um membro não pode receber um cargo de posição ≥ à sua posição mais alta (exceto o owner).
4. Uma mensagem não pode ter `reply_to` apontando para outro canal.
5. `deleted_at` preenchido implica que o `content` foi apagado (tombstone preserva apenas metadados).
6. Um usuário banido não pode existir em `server_members` do mesmo servidor.
7. Um canal de voz não pode conter `messages` de tipo `default` se `type = 'voice'` e o chat de voz estiver desabilitado.
8. `member_roles` só pode referenciar cargos do mesmo `server_id` do membro.
...
O documento original sugere duas trilhas. Ambas são válidas — a escolha depende do horizonte.
...
| Prós                                      | Contras                                                            |
| ----------------------------------------- | ------------------------------------------------------------------ |
| Auth, realtime e storage prontos no dia 1 | Realtime do Supabase é por tabela/linha, não por evento de domínio |
| RLS dá segurança no nível do banco        | Permissões complexas em RLS ficam lentas e difíceis de testar      |
| Time de 1–2 devs entrega em semanas       | Fan-out para 10k conexões vira caro rápido                         |
| Custo inicial próximo de zero             | Migração posterior é reescrita do gateway                          |

#### Trilha B — Arquitetura própria (NestJS + Gateway dedicado)

| Prós                                     | Contras                                             |
| ---------------------------------------- | --------------------------------------------------- |
| Controle total do protocolo de eventos   | 3–4 semanas a mais até a primeira mensagem trafegar |
| Fan-out otimizado, permissões em memória | Precisa gerenciar Redis, sessões, deploy de WS      |
| Custo por conexão muito menor em escala  | Exige mais maturidade de infra                      |

#### Recomendação

**Trilha híbrida.** Postgres gerenciado (Supabase ou Neon) como banco, mas **gateway WebSocket próprio desde o início**. Motivo: o banco é fácil de trocar depois; o protocolo de tempo real, não. Usar Supabase Realtime na fase 1 e migrar na fase 2 significa reescrever a camada de sincronização do cliente inteiro — exatamente a parte mais frágil e mais testada do sistema.

O que **sim** vale terceirizar desde o início: autenticação (Supabase Auth ou Auth.js), storage (R2), e SFU (LiveKit Cloud).
...
**Detalhe que mais causa bug:** dentro de um mesmo escopo, os `deny` e `allow` de _todos_ os cargos são acumulados **antes** de serem aplicados. Se você aplicar cargo a cargo em sequência, um `deny` do cargo Membro pode sobrescrever um `allow` do cargo Moderador dependendo apenas da ordem de iteração — comportamento não determinístico e impossível de explicar ao usuário.
...
Conexão: `wss://gateway.factory.app/?v=1&encoding=json` (`zlib-stream` opcional).
...
**Regra de agregação:** o status efetivo é o _mais disponível_ entre as sessões, exceto se o usuário escolheu manualmente — escolha manual sempre vence. Ao fechar a última sessão, o Gateway não emite `offline` imediatamente: aguarda 15 s. Isso elimina o flicker de "offline/online" durante um refresh de página.
...
| Escalada de privilégio       | Regras de hierarquia (§9.3) verificadas no servidor em toda mutação de cargo                                       |
...
Observações de direção visual: o tamanho base de texto é **15px**, não 16px — densidade importa numa interface de chat, e 15px é o ponto onde ainda há conforto de leitura com mais linhas visíveis. As superfícies usam neutros ligeiramente azulados; cinza puro (`#1a1a1a`) parece morto ao lado de conteúdo colorido. O acento é um índigo levemente dessaturado, escolhido para não competir com os avatares e cores de cargo dos usuários.
...
O diferencial do produto. Cada canal pode expor abas além do chat, e essas abas compartilham permissões, membros e contexto.
...
Apps rodam em `<iframe sandbox="allow-scripts allow-forms">` servido de `apps.factory.app` — **origem diferente** da aplicação principal. Sem `allow-same-origin`, o app não tem acesso a cookies, localStorage ou DOM do host. Toda comunicação passa por `postMessage` com validação de origem e de esquema.
...
```ts
import { createApp } from "@factory/sdk";
...
Monorepo, CI, schema base, autenticação (registro, login, refresh, MFA), design system inicial, deploy de staging.

**Aceite:** usuário registra, faz login, vê tela vazia autenticada; pipeline verde ponta a ponta.

### Fase 1 — MVP (6 semanas)

Servidores, categorias, canais de texto, mensagens em tempo real, reactions, respostas, menções, upload de imagens, lista de membros, presença, cargos básicos, convites, perfil.

**Aceite:**

- Dois usuários em navegadores distintos conversam com latência p95 < 500 ms.
- Convite funciona com validade e limite de usos.
- Cargo negando `VIEW_CHANNEL` esconde o canal em tempo real, sem refresh.
...
DMs e group DMs, amizades e bloqueio, threads, busca, notificações (in-app, desktop, push), moderação completa, audit log, permissões avançadas com overrides, emojis customizados, slowmode.

**Aceite:**

- Busca com `from:` e `in:` retorna resultado correto em < 500 ms sobre 1M mensagens.
...
Voz, vídeo, screen sharing, controles de dispositivo, layout dinâmico, stage channels, plataforma de apps + SDK, apps nativos (Files, Tasks, 3D Viewer, Meetings).

**Aceite:**

- 8 pessoas em voz com mouth-to-ear p95 < 200 ms.
- Screen share 1080p/30 estável por 30 min.
- Um app de terceiro roda em sandbox sem acesso ao token do usuário (verificado por pentest interno).
- 3D Viewer abre GLB de 50 MB em < 5 s.

### Fase 4 — Escala e plataforma (contínuo)

App mobile (React Native), desktop (Tauri), bots e webhooks, automoderação com ML, gravação e transcrição, monetização, E2E em DMs, marketplace de apps.
...
| --- | --------------------------------------------------------------- | -------- | ----------- | ------------------------------------------------------------------------------------ |
| R1  | Escopo cresce até virar clone completo do Discord               | **Alta** | **Alto**    | Fases com critério de aceite objetivo; nada entra na fase atual sem sair outra coisa |
| R2  | Custo de voz cresce mais rápido que a receita                   | Média    | Alto        | Limitar minutos no plano gratuito; monitorar custo por MAU semanalmente              |
| R3  | Bug de permissão vaza mensagem privada                          | Média    | **Crítico** | 100% de cobertura no motor; pentest antes de cada fase; bug bounty                   |
| R4  | Fan-out não escala em servidor grande                           | Média    | Alto        | Subscrição lazy desde o dia 1; teste de carga em cada fase                           |
| R5  | Complexidade da lista virtualizada trava o time                 | **Alta** | Médio       | Construir isolada em Storybook na semana 1 da fase 1                                 |
| R6  | Migrar de Supabase Realtime para gateway próprio vira reescrita | Média    | Alto        | **Evitado por decisão:** gateway próprio desde o início                              |
| R7  | Moderação insuficiente em comunidade pública                    | Média    | Alto        | Automoderação na fase 2; verificação obrigatória em servidores públicos              |
| R8  | Sandbox de apps é contornada                                    | Baixa    | **Crítico** | Origem separada, sem `allow-same-origin`; revisão de segurança obrigatória           |
| R9  | Nenhum diferencial percebido contra Discord                     | Média    | **Crítico** | Validar 3D Viewer e Tasks com 5 estúdios antes da fase 3                             |
| R10 | Time pequeno demais para 4 aplicações                           | **Alta** | Alto        | Terceirizar auth, storage e SFU; não construir mobile nativo na v1                   |
| R11 | Partição de mensagens não criada a tempo                        | Baixa    | Alto        | Job com 7 dias de antecedência + alerta P1                                           |
| R12 | Perda de dados por migration destrutiva                         | Baixa    | **Crítico** | Expand/contract obrigatório; PITR testado trimestralmente                            |

**Os três riscos que realmente decidem o projeto:** R1 (escopo), R3 (permissões) e R9 (diferencial). R1 mata por exaustão, R3 mata por incidente, R9 mata por indiferença do mercado.
...
| Tema                   | Fonte                                                            |
| ---------------------- | ---------------------------------------------------------------- |
| Snowflake IDs          | Twitter Engineering — _Announcing Snowflake_                     |
| Fan-out em escala      | Discord Engineering — _How Discord Stores Trillions of Messages_ |
| Presença distribuída   | Discord Engineering — _How Discord Maintains State_              |
| SFU e WebRTC           | LiveKit Docs — _Architecture_                                    |
| FTS em Postgres        | PostgreSQL Docs — cap. 12                                        |
| Segurança de iframe    | MDN — _iframe sandbox_ / OWASP Cheat Sheets                      |
| Virtualização de lista | TanStack Virtual — _Dynamic sizing_                              |
...
_Documento vivo. Toda decisão que contrarie uma ADR existente deve gerar uma nova ADR que a substitua explicitamente, com o motivo da mudança._
